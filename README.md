...
（restart-proxy）に対応。さらに **ESC＋十字キー（↑↓←→）のフローティング
操作パッド**を備え、物理キーの無いモバイルでも矢印/ESC を送れます
（`position:fixed` で native スクロールに追従、grip でドラッグ移動・
位置は記憶）。DECSET 2026 非対応の端末でもブラウザ側（`sync.js`）が
atomic 描画＝ちらつき無し。
